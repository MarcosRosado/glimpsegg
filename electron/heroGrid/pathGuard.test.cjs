...
const HOME = '/home/jogador';
const NATIVE = `${HOME}/.local/share/Steam`;
...
const fsImpl = {
  realpathSync: (p) => {
    if (p === `${HOME}/.steam/steam` || p === `${HOME}/.steam/root`) return NATIVE;
    return p;
  },
};
...
describe('assertAllowedGridPath — a guarda S-1', () => {
  ...
  it('aceita raiz que é symlink para uma raiz reconhecida (o caso da maquina real)', () => {
    expect(assertAllowedGridPath(grid(`${HOME}/.steam/steam`, 86738327), opts()).allowed).toBe(true);
  });

  it('aceita as raizes Flatpak e Snap do contrato', () => {
    const flatpak = `${HOME}/.var/app/com.valvesoftware.Steam/.local/share/Steam`;
    const snap = `${HOME}/snap/steam/common/.local/share/Steam`;
  ...
  it('recusa caminho solto, que nem tem a forma userdata/<id3>/570/remote/cfg', () => {
    const r = assertAllowedGridPath('/tmp/qualquer/hero_grid_config.json', opts());
    expect(r.allowed).toBe(false);
    expect(r.reason).toBe('BAD_SHAPE');
  });

  it('recusa caminho com a forma certa mas sob raiz Steam nao reconhecida', () => {
    const r = assertAllowedGridPath(grid('/opt/steam-pirata', 86738327), opts());
    expect(r.allowed).toBe(false);
  ...
  it('recusa raiz que apenas COMECA com o nome de uma raiz reconhecida', () => {
    const r = assertAllowedGridPath(grid(`${NATIVE}Evil`, 1), opts());
    expect(r.allowed).toBe(false);
    expect(r.reason).toBe('UNKNOWN_STEAM_ROOT');
  });

  describe('caminho manual (FR-006)', () => {
    const manual = '/mnt/disco2/SteamLibrary/hero_grid_config.json';
    ...
    it('recusa vizinho no mesmo diretorio do caminho manual', () => {
      const vizinho = '/mnt/disco2/SteamLibrary/outra/hero_grid_config.json';
