...
import type { SyncRecord, SyncState } from '../../../src/types/heroGrid';
import { MAX_SYNC_HISTORY } from '../../../src/utils/heroGrid/preferences';
import {
  BACKOFF_BASE_MS,
  BACKOFF_MAX_MS,
  backoffMs,
  ...
  syncFreshness,
  syncPhase,
} from '../../../src/utils/heroGrid/syncScheduler';
...
describe('syncScheduler: pureza', () => {
  it('nao chama Date.now() em nenhum ponto do modulo', () => {
    ...
    const source = readFileSync(new URL('../../../src/utils/heroGrid/syncScheduler.ts', import.meta.url), 'utf-8');
